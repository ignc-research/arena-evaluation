...
### load data ###
    ...
    def load_data(self):
        ...
        self.get_repetition_number() # self.repetition_numbers
    ...
    def get_repetition_number(self):
        repetition_numbers = ["rep0"+str(x) for x in range(100)] + ["rep"+str(x)for x in range(100,1000)]
        ...
        for map in self.maps:
            ...
            for key in self.keys:
                if self.data[key]["map"] == map:
                    map_keys.append(key) # append key if map matches current map
            for repetition_number in self.repetition_numbers:
                repetition_keys = [] # list of keys with current map
                for key in self.keys:
                    if self.data[key]["repetition_number"] == repetition_number:
                        repetition_keys.append(key) # append key if map matches current map
                for velocity in self.velocities:
                    vel_keys = [] # list of keys with current velocity
                    ...
                    for obstacle_number in self.obstacle_numbers:
                        ...
                        for metric in metrics:
                            ...
                            if self.config["plot_quantitative_suptitle"]:
                                plt.suptitle("{0}".format(self.config["plot_quantitative_labels"][metric].split("[")[0]), fontsize = self.config["plot_quantitative_suptitle_size"], fontweight = "bold")
                            # subtitle
                            if self.config["plot_quantitative_title"]:
                                # create subtitle based on scenario information
                                map_info = "Map: {}".format(map)
                                if repetition_number != "base_repetition_number":
                                    repetition_info = " Repetitions: {}".format(int(repetition_number.replace("rep","")))
                                else:
                                    repetition_info = ""
                                ...
                                if velocity != "base_velocity":
                                    ...
                                    velocity_info = ""
                                plt.title(map_info + repetition_info + obstacle_info + velocity_info, fontsize = self.config["plot_quantitative_title_size"])

                            plt.savefig(self.plot_dir + "/quantitative_plots/{0}_{1}_{2}_{3}_{4}".format(metric,map,obstacle_number,velocity,repetition_number), bbox_inches='tight')
                            plt.close()
